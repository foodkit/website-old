...
function initResetTabs() {
	jQuery('#v-pills-tabContent .btn-close-lightbox').on('click', function(e) {
		e.preventDefault();
		var tabPane = jQuery(this).closest('.active');
		tabPane.removeClass('active').attr('aria-expanded', 'false');
		jQuery('a[href="#'+ tabPane.attr('id') +'"]').removeClass('active').attr('aria-expanded', 'false');
...
function initLightbox() {
	var nav = jQuery('#v-pills-tabContent .tab-pane');

	ResponsiveHelper.addRange({
		'..767': {
			on: function() {
				nav.removeClass('active show').attr('aria-expanded', 'false');
				jQuery('#v-pills-tab .nav-link').removeClass('active').attr('aria-expanded', 'false');
			},
			off: function() {
				nav.addClass('fade').first().addClass('active show').attr('aria-expanded', 'true');
				jQuery('#v-pills-tab .nav-link').first().addClass('active').attr('aria-expanded', 'true');
			}
		}
	});
}
